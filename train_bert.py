...
from model.aagcn_bert import AAGCN_BERT
...
class Instructor:
    def __init__(self, opt):
        ...
        self.trainset = ABSADataset(opt.dataset_file['train'], tokenizer)  # 取数据集字典的键值，即数据集路径
        self.testset = ABSADataset(opt.dataset_file['test'], tokenizer)
        assert 0 <= opt.valset_ratio < 1 # =0表示不需要验证集
        # 是否需要验证集
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                # col为模型的输入，在batch中作为键(key)循环
                # 取出data_utils_bert中ABSADataset的data字典的键值(数据集经过处理的输入变量)
                inputs = [batch[col].to(self.opt.device) for col in self.opt.inputs_cols]

                outputs = self.model(inputs)  # [1,3]
                targets = batch['polarity'].to(self.opt.device) # [1]
...
def main():
    ...
    parser.add_argument('--dataset', default='toiletries', type=str,
                        help='')
    ...
    dataset_files = {
        'toiletries':{
            'train':'./datasets/ask-all/toiletries/toiletries_train.raw',
            'test':'./datasets/ask-all/toiletries/toiletries_test.raw'
        }

    }
    input_colses = {
        'aagcn_bert':['concat_question_indices', 'concat_answer_indices',
                      'concat_question_segments_indices', 'concat_answer_segments_indices',
                      'dependency_graph', 'question_dependency_graph', 'answer_dependency_graph']
